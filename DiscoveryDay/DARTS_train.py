...
def prepare_tensors(norm=None):
    if norm:
        ...
        X = np.load("DiscoveryDay/X_norm.npy", mmap_mode="r")
        y = np.load("DiscoveryDay/y_norm.npy", mmap_mode="r")
        ...
        X = np.load("DiscoveryDay/X_reg.npy", mmap_mode="r")
        y = np.load("DiscoveryDay/y_reg.npy", mmap_mode="r")
...
with open("DiscoveryDay/DARTS_genotypes.py") as graph_file:
    ...
    for i, g in enumerate(graphs):
        if i%2 != 0:
            ...
            plot(list(genotype.normal), "DiscoveryDay/DARTS_genotypes/normal", str(epoch+1))
...
filenames = ["DiscoveryDay/DARTS_genotypes/" + f for f in sorted_alphanumeric(os.listdir("DiscoveryDay/DARTS_genotypes")) if f.endswith(".png")]

def make_gif(frame_folder):
    frames = [Image.open(image) for image in frame_folder]
    frame_one = frames[0]
    frame_one.save("/home/msayler/DARTS_gitRepo/Transformer_Algorithms/DiscoveryDay/genotypes.gif", format="GIF", append_images=frames,
...
# DARTS_model.summary()
plot_model(DARTS_model, to_file='DiscoveryDay/DARTS_train_model.png', expand_nested=True, show_shapes=True)
...
checkpoint_filepath = "DiscoveryDay/DARTS_train_checkpoints/checkpoints"    # path to save checkpoint weights
...
with open('DiscoveryDay/DARTS_train_history', 'wb') as file_pi:    # path to save model history
    pickle.dump(transformer_model_history.history, file_pi)

with open('DiscoveryDay/DARTS_train_history', "rb") as file_pi:    # path to load model history
...
DARTS_model.load_weights(checkpoint_filepath)
DARTS_model.save('DiscoveryDay/DARTS_train_model')  # path to save complete model

# DARTS_model = load_model('DiscoveryDay/DARTS_train_model.keras')   # path of complete model
