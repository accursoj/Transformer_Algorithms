...
plot_model(sna_model, to_file = 'DiscoveryDay/DARTS_model.png', expand_nested=True, show_shapes=True)
...
with strategy.scope(): 
    ...
    for epoch in range(cfg["epoch"]):
        ...
        if epoch >= 5:
            ...
            f = open('DiscoveryDay/DARTS_genotypes.py', 'a')
            f.write(f"\n{cfg['sub_name']}_{epoch} = {genotype}\n")
            f.close()

sna_model.save('DiscoveryDay/DARTS_model')
...
with open('DiscoveryDay/DARTS_history', 'wb') as file_pi:    # path to save model history
    pickle.dump(search_history, file_pi)

np.save("DiscoveryDay/DARTS_history.npy", search_history)
...
train_curves(search_history, "DARTS Model:")
